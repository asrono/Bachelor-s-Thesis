...
j1 = double2single_index(6,2);
j2 = double2single_index(4,2);
a_vec_test = zeros(1,j1);
a_vec_test(j1+1) = 1;
a_vec_test(j2+1) = 1;

b_vec = Zernike_laplacian_Janssen_vec(a_vec_test);

for j = 0:(length(b_vec)-1)
...
plot(rplot,200*surface,'-k','LineWidth',4);

surface4 = coef2surf_laplacian2(a_vec,rplot);
plot(rplot,surface4,'-g','LineWidth',4);

surface6 = coef2surf(Zernike_laplacian_Janssen_vec(a_vec),rplot);
plot(rplot,surface4,':r','LineWidth',4);

surface7 = coef2surf(Zernike_inverse_laplacian_Janssen_vec(Zernike_laplacian_Janssen_vec(a_vec)),rplot);
plot(rplot, 200*surface7,':r','LineWidth',4);
...
% calculate over different z
I = 1 - (n - 1)*z'.*coef2surf(Zernike_laplacian_Janssen_vec(a_vec),rplot);
I2 = 1./ (1 + (n - 1)*z'.*coef2surf(Zernike_laplacian_Janssen_vec(a_vec),rplot));  

% calculate Zernike coef at certain z
zf = xf;

a_vec_intensity = Zernike_laplacian_Janssen_vec((1-n)*zf*a_vec);
a_vec_intensity(1) = a_vec_intensity(1) + 1;

figure(2);
plot(rplot,I(end,:),'k','LineWidth',4); hold on;
plot(rplot,coef2surf(a_vec_intensity,rplot),':g','LineWidth',4);

plot(rplot,I2(end,:),'b','LineWidth',4); hold on;
legend({'linear approx','linear approx Janssen','non linear approx'},...
        'Interpreter','latex',...
        'Location', 'northeast');
